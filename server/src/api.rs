...
pub async fn post_sync(
    ...
    match db::store_sync(&conn, &body) {
        Ok(n) => {
            log::info!(
                "Sync received | machine_id={} sales={} events={} metrics={} new_records={}",
                body.machine_id,
                body.sales.len(),
                body.events.len(),
                body.metrics.len(),
...
pub async fn get_metrics(
    ...
    let machine_id = query.machine_id.unwrap_or(1);
    ...
    let all_rows: Vec<MetricRow> = stmt
        .query_map(params![machine_id, neg], |row| {
            Ok(MetricRow {
                timestamp:        row.get(0)?,
    ...
    // Online = last metric received within the past 2 minutes
    ...
    // Detect offline gaps (> 2 min between consecutive readings)
